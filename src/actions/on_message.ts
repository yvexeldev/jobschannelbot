  ...
  if (elon) {
    ...
    if (elon_state == 'name') {
      ...
      if ('text' in ctx.message) {
        ...
        await elon.save();
        if (lang === 'UZB') {
          let txt = "<b>Hozirgi o'qish yoki ish joyingiz:</b> (namuna: Najot Talim talabasi)";
          ...
          let txt = '<b>Ваше текущее место учебы или работы:</b> (обр.: Слушатель Najot Talim)';
...
composer.action(/.+/, async (ctx) => {
  ...
  if (message.slice(0, 2) == 'ok') {
    ...
    if (elon) {
      ...
      await ctx.telegram.sendMessage(`${elon.user_id}`, `${msgText} https://t.me/RitnyxJobs/${post.message_id}`, {
        parse_mode: 'HTML',
      });
